...
def test_get_resume_panel_lists_existing_resume(backend_client, requests_mock, auth_headers):
    ...
    requests_mock.get(f"{DB_SERVICE_URL}/profiles/10/resumes", json=[{"resume_id": 5, "file_name": "resume.pdf", "file_type": "application/pdf"}])
...
def test_upload_resume_rejects_disallowed_type(backend_client, requests_mock, auth_headers):
    mock_session(requests_mock, APPLICANT)
    requests_mock.get(f"{DB_SERVICE_URL}/profiles/by-user/1", json={"profile_id": 10, "user_id": 1})
    requests_mock.get(f"{DB_SERVICE_URL}/profiles/10/resumes", json=[])
...
def test_upload_resume_shows_duplicate_error(backend_client, requests_mock, auth_headers):
    ...
    requests_mock.get(f"{DB_SERVICE_URL}/profiles/by-user/1", json={"profile_id": 10, "user_id": 1})
    requests_mock.post(
        f"{DB_SERVICE_URL}/profiles/10/resumes",
...
def test_delete_resume_forbidden_for_non_owner(backend_client, requests_mock, auth_headers):
    ...
    requests_mock.get(f"{DB_SERVICE_URL}/profiles/by-user/1", json={"profile_id": 11, "user_id": 1})
