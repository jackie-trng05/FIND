...
#     entered." The overall score is the average of the five criteria. These
#     tests prove the calculation is correct for all five criteria and returns
#     well within the 2-second budget. (The form also computes it client-side
#     in updateOverall(); see test_frontend.py for that wiring.) ---

import time


def test_overall_score_is_average_of_all_five(db_client):
    resp = _create(
        db_client,
        Evaluation_TechnicalScore=5,
        Evaluation_EducationScore=5,
        Evaluation_CommunicationScore=4,
        Evaluation_ProblemSolvingScore=4,
        Evaluation_ProfessionalismScore=2,
        Evaluation_FinalRecommendation="Hire",
    )
    assert resp.status_code == 201
    # (5 + 5 + 4 + 4 + 2) / 5 == 4.0
    assert resp.get_json()["Evaluation_OverallScore"] == 4.0


def test_overall_score_calculated_within_2_seconds(db_client):
    start = time.perf_counter()
    resp = _create(
        db_client,
        Evaluation_TechnicalScore=3,
        Evaluation_EducationScore=4,
        Evaluation_CommunicationScore=5,
        Evaluation_ProblemSolvingScore=2,
        Evaluation_ProfessionalismScore=1,
        Evaluation_FinalRecommendation="Reject",
    )
    elapsed = time.perf_counter() - start
    assert resp.status_code == 201
    assert resp.get_json()["Evaluation_OverallScore"] == 3.0
    assert elapsed < 2.0, f"overall score took {elapsed:.3f}s, NFR budget is 2s"
